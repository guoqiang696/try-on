...
(function () {
  ...
  async function squareApi(path, options = {}) {
  ...
  async function loadServerPosts() {
    try {
      ...
      if (state.tab && state.tab !== 'following') params.set('tab', state.tab);
      if (state.search.trim()) params.set('q', state.search.trim());
      if (state.chip && !['可保存模特', '可复用风格', '男装', '女装'].includes(state.chip)) params.set('tag', state.chip);
      const data = await squareApi(`/api/square${params.toString() ? `?${params}` : ''}`);
  ...
  function resolveGalleryWork(workId) {
    if (GALLERY_WORKS[workId]) return GALLERY_WORKS[workId];
    const card = document.querySelector(`[data-opc-work-id="${CSS.escape(String(workId))}"]`);
    if (!card) return null;
    const image = card.querySelector('.opc-gallery-card-media img')?.src || '';
    const title = card.querySelector('.opc-gallery-card-title')?.textContent?.trim() || 'AI 试衣作品';
    const tags = Array.from(card.querySelectorAll('.opc-gallery-card-tags .opc-tag')).map((el) => el.textContent.trim()).filter(Boolean);
    return {
      title,
      image,
      type: 'tryon',
  ...
  async function toggleLike(id) {
    ...
    savePosts(posts);
    try {
      await squareApi(`/api/square/${id}/like`, {
        method: 'POST',
        body: JSON.stringify({ active: idx < 0 }),
      });
      await loadServerPosts();
    } catch {
      // Local optimistic update remains available offline.
    }
  }
  ...
  async function toggleSave(id) {
    ...
    savePosts(posts);
    try {
      await squareApi(`/api/square/${id}/save`, {
        method: 'POST',
        body: JSON.stringify({ active: idx < 0 }),
      });
      await loadServerPosts();
  ...
  async function saveModelFromPost(post) {
    ...
    try {
      await squareApi(`/api/square/${post.id}/save-model`, { method: 'POST' });
    } catch {
  ...
  async function renderSquareHome() {
    await loadServerPosts();
  ...
  async function renderSquareDetail() {
  ...
  function renderSquarePublish() {
    ...
    const work = workId ? resolveGalleryWork(workId) : null;
  ...
  async function publishWork(workId) {
    const work = resolveGalleryWork(workId);
    if (!work) return;
    ...
    try {
      const payload = {
        result_id: /^\d+$/.test(String(workId)) ? Number(workId) : null,
        post_type: type,
        title,
        description: desc,
        image: work.image,
        model: work.model,
        tags: [...state.publishTags],
        allow_save_model: allowSaveModel && !!work.model,
        allow_remix: allowRemix,
        style_prompt: work.stylePrompt || title,
      };
      const data = await squareApi('/api/square', {
        method: 'POST',
      ...
      // Keep the local post as an offline fallback.
    }
  ...
  async function addComment(postId, text) {
    ...
    savePosts(posts);
    try {
      await squareApi(`/api/square/${postId}/comments`, {
        method: 'POST',
        body: JSON.stringify({ text: text.trim() }),
      });
      await loadServerPosts();
    } catch {
      // Local comment remains visible offline.
    }
    renderSquareDetail();
  }
  ...
  async function handleClick(e) {
    ...
    if (savePostBtn) {
      e.preventDefault();
      e.stopPropagation();
      await toggleSave(savePostBtn.getAttribute('data-square-save-post'));
      onRouteChange(window.OPC?.parseRoute?.() || 'square');
      return;
    }
    ...
    if (saveModelBtn) {
      ...
      const result = await saveModelFromPost(post);
      if (result === 'ok') {
  ...
  async function handleSubmit(e) {
    if (e.target.id === 'opc-square-publish-form') {
      e.preventDefault();
      await publishWork(e.target.getAttribute('data-work-id'));
    }
  }
