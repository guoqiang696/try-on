/**
 * OPC 智能试衣 — 单页 Hash 路由
 * #home #model #models #real #free #gallery #square #profile
 */
(function () {
  const TRY_ROUTES = new Set(['model', 'models', 'real', 'free', 'model-library']);
  const SQUARE_ROUTES = new Set(['square', 'square-detail', 'square-publish']);
  ...
  const ROUTES = {
    ...
    'model-library': { title: '我的模特库', top: 'try', sidebar: 'model' },
    real: { title: '真人照片试穿', top: 'try', sidebar: 'real' },
    free: { title: '自由风格生成', top: 'try', sidebar: 'free' },
    square: { title: '广场', top: 'square' },
    'square-detail': { title: '作品详情', top: 'square' },
    'square-publish': { title: '发布到广场', top: 'square' },
  ...
  function updateBottomNav(route) {
    ...
    else if (SQUARE_ROUTES.has(route)) bottomKey = 'square';
  ...
  function navigate(route, scrollTargetOrQuery) {
    ...
    let query = null;
    if (scrollTargetOrQuery && typeof scrollTargetOrQuery === 'object' && !Array.isArray(scrollTargetOrQuery)) {
      query = scrollTargetOrQuery;
    } else if (scrollTargetOrQuery) {
      scrollTarget = scrollTargetOrQuery;
    }
  ...
  window.OPC = { navigate, parseRoute, parseQuery, render };
